...
public class QueueIntFlip {
	...
	public int put(int[] newElements, int length) {
		...
		if (!flipped) {
			...
			if (length <= capacity - writePos) {
				...
				// new elements must be divided between top and bottom of
				// elements array
	...
	public int take(int[] into, int length) {
		int intoWritePos = 0;
		if (!flipped) {
			// writePos higher than readPos - available section is writePos -
			// readPos
			...
			// readPos higher than writePos - available sections are top +
			// bottom of elements array

			if (length <= capacity - readPos) {
				// length is lower than the elements available at the top of the
				// elements array - copy directly
				...
				// length is higher than elements available at the top of the
				// elements array
				// split copy into a copy from both top and bottom of elements
				// array.
