...
public class MessageBuffer {
	...
	byte[] smallMessageBuffer = new byte[1024 * 4 * KB]; // 1024 x 4KB messages
															// = 4MB.
	byte[] mediumMessageBuffer = new byte[128 * 128 * KB]; // 128 x 128KB
															// messages = 16MB.
	byte[] largeMessageBuffer = new byte[16 * 1 * MB]; // 16 * 1MB messages =
														// 16MB.

	QueueIntFlip smallMessageBufferFreeBlocks = new QueueIntFlip(1024); // 1024
																		// free
																		// sections
	QueueIntFlip mediumMessageBufferFreeBlocks = new QueueIntFlip(128); // 128
																		// free
																		// sections
	QueueIntFlip largeMessageBufferFreeBlocks = new QueueIntFlip(16); // 16 free
																		// sections
	...
	public Message getMessage() {
		...
		Message message = new Message(this); // todo get from Message pool -
												// caps memory usage.
