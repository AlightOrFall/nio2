...
public class SocketProcessor implements Runnable {
	...
	private MessageBuffer readMessageBuffer = null; // todo Not used now - but
													// perhaps will be later -
													// to check for space in the
													// buffer before reading
													// from sockets
	private MessageBuffer writeMessageBuffer = null; // todo Not used now - but
														// perhaps will be later
														// - to check for space
	...
	private Queue<Message> outboundMessageQueue = new LinkedList<Message>(); // todo
	// use a
	// better
	// /
	// faster
	...
	private long nextSocketId = 16 * 1024; // start incoming socket ids from 16K
											// - reserve bottom ids for
											// pre-defined sockets (servers).
	...
	private void readFromSocket(SelectionKey key) throws IOException {
		...
		if (fullMessages.size() > 0) {
			for (Message message : fullMessages) {
				...
				this.messageProcessor.process(message, this.writeProxy); // the
																			// message
																			// processor
																			// will
																			// eventually
		...
		while (outMessage != null) {
			...
			if (socket != null) {
				...
				if (messageWriter.isEmpty()) {
					...
					emptyToNonEmptySockets.add(socket); // not necessary if
														// removed from
